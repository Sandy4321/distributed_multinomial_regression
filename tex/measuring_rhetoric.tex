...
\vskip .25cm
{\bf \LARGE \theme Measuring Rhetoric \bk\vskip .2cm \Large Statistical Language Models in Social Science }
...
{\gr empirical evidence, quantification of social concepts...}
...
\vskip .25cm
{\color<2->{gray}{Techniques from stats and ML are beginning to filter through and researchers are estimating relationships {\it from the data}.}}

\vskip .25cm  
\only<2>{
Describe topic models
}


\onslide<3>{ {\it Grimmer AJPS 2013:}  fit latent topics in press releases (e.g., `Iraq', `Infastructure') then investigate who uses what topic.
...
\begin{frame}
Regression instead of latent structure.  observables.  partial correlation.

\end{frame}

\begin{frame}

making it work: distribution.  reg tools like anywhere (penalty).
