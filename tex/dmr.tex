...
\cline{2-6}
MNIR + lasso & non-vote attributes, $\bm{C}$ & 25,876 & 0.308 & 0.291 & 0.339\\
standard lasso & non-vote attributes, $\bm{z}$, $\bm{m}$ & 11,940 & 0.316 & 0.296 & 0.341
\end{tabular}
\end{center}
\vskip -.25cm
...
estimate.  The problem is that you want to know what will happen to $y$ if $d$
changes independently from  the other attributes, even though everything is
collinear in the data and both $y$ and $d$ could be correlated to other
unobserved confounders.  The best that you can do is to estimate the treatment
effect -- that of $d$ on $y$ -- while controlling for observable potential
confounders.  In text analysis, this includes controlling for the
text content itself.
...
they tend to give more positive reviews, perhaps because new users only bother
to give negative feedback or because the community norms encourage a high
average rating? It is hard to imagine getting firm evidence in either
direction without running a randomized trial -- we will always be worried
about the effect of an omitted confounder.  However, we can try our best and
condition on available information.    In particular, we can condition on
review content to ask the question: even given the same review message, would
an experienced user give more or less stars than a newbie?
