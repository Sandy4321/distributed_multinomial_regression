...
function of only parameters (not $\bm{c}_i)$.  For example, either the full
multinomial logistic regression model (conditional upon $m_i$) or our
independent Poissons (conditional upon $\hat\mu_i$) can be written as in
...
We call $\bm{z}_i = \bs{\Phi}\bm{c}_i$ an {\it SR projection}. In practice, we
...
as a traditional sufficient statistic (like $\bar y$). For example, to predict
...
with highest values for both $a_{\tt funny}$ and $a_{\tt useful}$ (recall that
...
ignored unless we want to reduce bias in estimation of coefficients on
$\bm{u}_i$. Two algorithms are applied for predicting $\log(1+\bm{y})$ from
$[\bm{U},\bm{Z}^v,\bm{m}]$: ordinary least-squares (OLS), and a 1024 tree
random forest \citep[RF;][]{breiman_random_2001} as implemented in the
...
\begin{figure}
\begin{verbatim}
 funny.dtm    funny.z useful.dtm   useful.z   cool.dtm     cool.z 
     0.308      0.316      0.291      0.296      0.339      0.341 
\end{verbatim}
\caption{\label{yelp_cv}}
\end{figure}
